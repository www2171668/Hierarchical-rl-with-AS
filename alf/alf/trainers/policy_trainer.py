...
from alf.bin.Q_map import get_Q_map
from alf.bin.Q_fetch import get_Q_fetch
from alf.bin.Q_goal import get_Q_goal
...
class Trainer(object):
    ...
    def __init__(self, config: TrainerConfig, ddp_rank: int = -1):
        ...
        self._ignored_parameter_prefixes = []
        if config.para_prefixes != 0:
            self._ignored_parameter_prefixes = ["_skill_generator._rl"]

        self._including_optimizer = config.including_optimizer
        self._including_replay_buffer = config.including_replay_buffer
    ...
    def _save_checkpoint(self):
        """Saving is only enabled when running single process training(rank is -1)
        or master process of DDP training (rank is 0)."""
        if self._rank <= 0:
    ...
    def _restore_checkpoint(self, checkpointer):
        ...
        try:
            recovered_global_step = checkpointer.load(
                ignored_parameter_prefixes=self._ignored_parameter_prefixes,
                including_optimizer=self._including_optimizer,
                including_replay_buffer=self._including_replay_buffer,
                strict=self._strict)
            ...
            raise RuntimeError(
                ("Checkpoint loading failed from the provided root_dir={}. "
                 ...
                 "Use a new value for it if planning to train from scratch. \n"
...
class RLTrainer(Trainer):
    ...
    def __init__(self, config: TrainerConfig, ddp_rank: int = -1):
        ...
        if ddp_rank >= 0:
            if not self._algorithm.on_policy:
                raise RuntimeError('Mutli-GPU with DDP does not support off-policy training yet')
    ...
    def _train(self):
        ...
        while True:
            ...
            if ((self._num_iterations and iter_num >= time_to_checkpoint)
                    or (self._num_env_steps and total_time_steps >= time_to_checkpoint)):
                self._save_checkpoint()
    ...
    def _eval(self):
        ...
        policy_state = self._algorithm.get_initial_predict_state(self._eval_env.batch_size)
        trans_state = self._algorithm.get_initial_transform_state(self._eval_env.batch_size)
        episodes = 0
...
@torch.no_grad()
def _step(algorithm, env, time_step, policy_state, trans_state,
          metrics, render=False, recorder=None, sleep_time_per_step=0):
...
def play(root_dir,
    ...
    while episodes < num_episodes:
        ...
        if time_step.is_last():
            if reward_log:
                if 'success' in time_step.env_info:
                    success = time_step.env_info['success'][0].item()
                    logging.info("episode_length=%s episode_reward=%s episode_info=%s" %
                                 (episode_length, episode_reward, success))
                else:
