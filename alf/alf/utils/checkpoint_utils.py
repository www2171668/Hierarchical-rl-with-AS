...
class Checkpointer(object):
    ...
    def __init__(self, ckpt_dir, **kwargs):
        ...
        Args:
            ckpt_dir: The directory to save checkpoints. Create ckpt_dir if
                it doesn't exist.
            kwargs: [algorithm,metrics,trainer_progress]
                Items to be included in the checkpoint. Each item needs
    ...
    def load(self,
        ...
        def _load_one(module, checkpoint):
            ...
            if isinstance(module, nn.Module):
                missing_keys, unexpected_keys = module.load_state_dict(
                    checkpoint, strict=strict)  # * True:严格匹配 False:非严格匹配
            else:
        ...
        # % 加载终止模型,替换掉全局步数.如果没有latest模型,会用最大数来替换
        ...
        f_path = os.path.join(self._ckpt_dir, "ckpt-{0}".format(global_step))  # 模型名字
        ...
        # % 读取优化器和经验池  train:True  play:False
        ...
        for k in self._modules.keys():
            _remove_ignored_parameters(checkpoint[k])  # 移除要忽略的参数 "_skill_generator._rl"
            _convert_legacy_parameter(checkpoint[k])  # ALF版本匹配
    ...
    def save(self, global_step):
        """Save states of all modules to checkpoint.
           保存的是pkl文件的zip,仅保存参数,用net.state_dict()调取模型参数
        ...
        state = {
            ...
            if type(v) == torch.nn.DataParallel else v.state_dict()  # 存入参数
