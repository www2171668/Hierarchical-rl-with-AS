...
class Normalizer(nn.Module):
    ...
    def update(self, tensor):
        ...
        """
        if self._mean_averager:  # 更新正则化所用均值
            self._mean_averager.update(tensor)
        if self._m2_averager:  # 更新正则化所用方差
    ...
    def _normalize(self, tensor, clip_value=-1.0):
        def _normalize(m2, t, m):
            ...
            if m2 is not None:
                if m is not None:
                    var = torch.relu(m2 - math_ops.square(m))  # var - m^2 通过relu消除负值
                else:
                    var = m2
            ...
            t = alf.layers.normalize_along_batch_dims(
                t, m, var, variance_epsilon=self._variance_epsilon)  # 因variance而并被放大1e5
            if clip_value > 0:
                t = torch.clamp(t, -clip_value, clip_value)  # normalize发挥了主要作用,很少会被clamp
            return t

        m2 = (self._m2_averager.get() if self._m2_averager else None)  # 获取方差.此处的方差和均值均是被_variance_epsilon放大过的
        m = (self._mean_averager.get() if self._mean_averager else None)  # 获取均值
