...
.. code-block:: bash
    ...
    --gin_file=sac_map_b.gin
    --root_dir=~/Project/ALF/alf/alf/runing/sac_map_b
    --checkpoint_step=10089
    --gin_param=suite_maps.load.expand_states=False
"""
...
def _define_flags():
    ...
    flags.DEFINE_integer('append_blank_frames', 0,
    ...
    flags.DEFINE_string('record_file', None,
                        "If provided, video will be recorded to a file instead of shown on the screen.")
    ...
    flags.DEFINE_bool('rgb', False, '渲染Mujoco图片，不能和render同时使用，还需要注释掉bashrc中的LD_PRELOAD')
    flags.DEFINE_bool('draw_q', False, '保存Q图所用数据')
    flags.DEFINE_bool('reward_log', False, '记录成功率')
...
def play():
    ...
    project_path = f"{project_path}/0.0"
    _setup_logging(log_dir=project_path, rank=0)
    logging.info("==> Begin Playing")
    try:
        policy_trainer.play(
