...
class RandomCategoricalGoalGenerator(RLAlgorithm):
    ...
    def _generate_goal(self, observation, state):
        ...
        goals = torch.randint(
            high=self._num_of_goals, size=(batch_size, ), dtype=torch.int64)  # 从均匀分布中采样
    ...
    def _update_goal(self, observation, state, step_type):
        ...
        """
        new_goal_mask = torch.unsqueeze((step_type == StepType.FIRST), dim=-1)  # 判定是否情节开始
        generated_goal = self._generate_goal(observation, state)  # 新目标
