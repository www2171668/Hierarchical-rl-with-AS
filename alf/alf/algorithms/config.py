    ...
    def __init__(self,
        ...
        Args:
            ...
            unroll_length (int):  number of time steps each environment proceeds per
                ...
                异策略中表示行走步数. 该步数结束时 -> 训练 -> tensorboard记录
            ...
            temporally_independent_train_step (bool|None): If True, the ``train_step``
                ...
                used if the algorithm has no state(RNN_state). For Algorithm with state
                (e.g. ``SarsaAlgorithm`` not using RNN), if there is no need to
        ...
        self.para_prefixes = para_prefixes
        self.high_initial_collect_steps = initial_collect_steps * 5
        self.including_optimizer = including_optimizer
        self.including_replay_buffer = including_replay_buffer
        self.strict = strict
